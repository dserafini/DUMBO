...
def writeregister(addr,data):	#Write ALPIDE register
	hw.getNode("cmd_addr.WR_addr").write(int(addr))
	hw.getNode("cmd_addr.WR_data").write(int(data))
	hw.dispatch()
	send_cmd(write_register)

def readregister(addr):	#Read ALPIDE register
	hw.getNode("cmd_addr.RR_addr").write(int(addr))
...
def mask_noisy_pixels():
	...
	writeregister(0x0005, 1000)	#STROBE duration 16 clk cycles (10us) (Not true)
	writeregister(0x0006, 20000)	#gap between 2 STROBES	384 clk cycles (240us) sample every 250 us-->4kHz
